...
import { ArrowLeft, Send, FileText, QrCode, ImageIcon, BookOpen } from 'lucide-react';
import { usePageDetail, useUpdatePage, usePublishPage } from '@/lib/hooks';
...
export function PageEditPage() {
  ...
  const { data: page, isLoading, error } = usePageDetail(id);
  ...
  const [initialBiographyJson, setInitialBiographyJson] = useState<Record<string, unknown> | null>(null);
  ...
  const hasBiographyChanges =
    JSON.stringify(biographyJson) !== JSON.stringify(initialBiographyJson);

  const hasAnyChanges = isDirty || hasBurialChanges || hasBiographyChanges;

  useEffect(() => {
    if (page) {
      reset({
        fullName: page.person.full_name,
        lifeStatus: page.person.life_status,
        gender: page.person.gender,
      ...
      const bioJson = (page.biography_json as unknown as Record<string, unknown>) || null;
      setBiographyJson(bioJson);
      setInitialBiographyJson(bioJson);
      const coords = {
        lat: page.person.burial_place_lat || null,
        lng: page.person.burial_place_lng || null,
      };
      setBurialCoords(coords);
      setInitialBurialCoords(coords);
      const photoUrl = page.person.burial_photo_url || null;
  ...
  if (error) {
    ...
    return (
      <div className="glass-card p-8 text-center">
        <div className="text-5xl mb-4">{is403 ? 'Denied' : 'Warning'}</div>
        <h2 className="text-xl font-semibold text-white mb-2">
          {is403 ? 'Доступ запрещён' : 'Ошибка загрузки'}
        </h2>
        <p className="text-zinc-400 mb-6">
          {is403 ? 'У вас нет доступа к этой странице' : error.detail}
        </p>
        <Button variant="secondary" onClick={() => navigate('/app/pages')}>
          Вернуться к списку
        </Button>
      </div>
    );
  ...
  return (
    <div className="space-y-6">
      <PageHeader
        title={page.title || page.person.full_name}
      ...
      <Tabs defaultValue="details" className="space-y-6">
        ...
        <TabsContent value="content">
          <div className="space-y-6">
            ...
            <LifeEventsSection pageId={id!} />
            <AchievementsSection pageId={id!} />
            <EducationSection pageId={id!} />
            <CareerSection pageId={id!} />
            <ValuesSection pageId={id!} />
            <QuotesSection pageId={id!} />
            <MemorialMessagesSection pageId={id!} />
          </div>
        </TabsContent>
      </Tabs>
      ...
      <ConfirmDialog
        ...
        message="После публикации страница станет доступна по публичной ссылке. Вы сможете редактировать её в любое время."
